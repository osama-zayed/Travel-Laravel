...
<div class="page-header">
    ...
    <div class="col-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">{{__('admin.editeNews')}}</h4>
                <form method="POST" action="{{route('admin.news_1.update', $news->id)}}"  class="forms-sample"  enctype="multipart/form-data" >
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="exampleInputName1">{{__('admin.address')}}</label>
                        <input type="text" class="form-control" name="address" value="{{ $news->address }}"  placeholder="{{__('admin.address')}}">
                        @error('address')
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="content"  placeholder="{{__('admin.content')}}"  rows="4">{{ $news->content }}</textarea>
                        @error('content')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                    </div>
